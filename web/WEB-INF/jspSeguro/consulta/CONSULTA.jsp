...
boolean cajeroConsultaSIR = false;
	
while (itRoles.hasNext())
{
	Rol rol = (Rol) itRoles.next();
	if (rol.getRolId().equals(CRoles.SIR_ROL_CONSULTA_SIR))
	{
		cajeroConsultaSIR = true;
	}
}

List alcances= new ArrayList();
//quitar el alcance nacional en caso de que no tenga el usuario de CAJERO_SIR o tenga rol de usuario operativo
if((!(cajeroConsultaSIR)) || (rol1.getRolId().equals(AWLiquidacionConsulta.ROL_USUARIO_OPERATIVO) || rol1.getRolId().equals(AWLiquidacionConsulta.ROL_USUARIO_OPERATIVO_CONSULTAS))){
...
if (tipoConsulta != null && !tipoConsulta.equals("")) {
  ...
  {
    tipoConsulta = AWLiquidacionConsulta.LIQUIDAR_CONSULTA_SIMPLE;
  } else if (tipoConsulta.equals(TipoConsulta.TIPO_COMPLEJO)) {
    tipoConsulta = AWLiquidacionConsulta.LIQUIDAR_CONSULTA_COMPLEJA;
  } else {
    tipoConsulta = "";
  }
}
        ...
        <tr>
          ...
          <td background="<%=request.getContextPath()%>/jsp/images/tabla_central_bgn003.gif"> <table border="0" cellpadding="0" cellspacing="0">
              <tr>
                ...
                <%if(rol1.getNombre().equals("CAJERO CONSULTAS") ){  %>              
    ...
    <!-- fwtable fwsrc="SIR_central_pantallasinternas_subsecciones.png" fwbase="sub.gif" fwstyle="Dreamweaver" fwdocid = "742308039" fwnested="1" -->
		<td align="left">
			...
			<tr>
				...
				<td><a href="consultas_simplificada.view"><img src="<%=request.getContextPath()%>/jsp/images/btn_short_anadir.gif" width="35" height="25" border="0"></a></td>
			</tr>
		</td>
			</table>
	</table>
...
%>
            ...
            <tr>
              <td><input name="<%= AWConsulta.TIPO_CONSULTA %>" type="radio"
                onClick="cambiarAccion('<%=AWLiquidacionConsulta.LIQUIDAR_CONSULTA_SIMPLE%>');document.BUSCAR.<%= CSolicitudConsulta.NUMERO_MAX_BUSQUEDAS %>.disabled = false;"
            ...
            <tr>
              <td><input name="<%= AWConsulta.TIPO_CONSULTA %>" type="radio"
                onClick="cambiarAccion('<%=AWLiquidacionConsulta.LIQUIDAR_CONSULTA_COMPLEJA%>');document.BUSCAR.<%= CSolicitudConsulta.NUMERO_MAX_BUSQUEDAS %>.disabled = false"
                <%  tipo = request.getSession().getAttribute(AWConsulta.TIPO_CONSULTA);
                    if (tipo != null && ((String)tipo).equals("2")) {%>checked<%}%>
                value="<%= TipoConsulta.TIPO_COMPLEJO %>"></td>
              <td>Consulta compleja (m&aacute;s de 3)</td>
              <td><span class="titulotbcentral"><img src="<%=request.getContextPath()%>/jsp/images/ico_flecha.gif" width="16" height="16"></span></td>
              <td class="campostip04">M&aacute;s de tres b&uacute;squedas</td>
            </tr>
            <tr>
              <td><input name="<%= AWConsulta.TIPO_CONSULTA %>" type="radio"
                onClick="cambiarAccion('<%=AWLiquidacionConsulta.LIQUIDAR_CONSULTA_CONSTANCIA%>');document.BUSCAR.<%= CSolicitudConsulta.NUMERO_MAX_BUSQUEDAS %>.disabled = true;"
                <%  tipo = request.getSession().getAttribute(AWConsulta.TIPO_CONSULTA);
                    if (tipo != null && ((String)tipo).equals("7")) {%>checked<%}%>
                    value="<%= TipoConsulta.TIPO_CONSTANCIA %>"></td>
              <td>Constancia de no propiedad</td>
              <td>&nbsp;</td>
              <td>&nbsp;</td>
            </tr>
...
if(! (rol.getRolId().equals(AWLiquidacionConsulta.ROL_USUARIO_OPERATIVO) || rol1.getRolId().equals(AWLiquidacionConsulta.ROL_USUARIO_OPERATIVO_CONSULTAS))){
...
%>
          ...
          <table width="100%" class="camposform">
            <tr>
              ...
              <td class="campostip04">Debe proporcionar el N&uacute;mero M&aacute;ximo de B&uacute;squedas a realizar</td>
            </tr>
...
%>
          ...
          <hr class="linehorizontal">
              ...
              <table width="100%" border="0" cellpadding="0" cellspacing="0">
                <tr> 
                  <td width="12"><img name="sub_r1_c1" src="<%=request.getContextPath()%>/jsp/images/sub_r1_c1.gif" width="12" height="22" border="0" alt=""></td>
                  <td class="bgnsub">Imprimir Recibo </td>
              ...
              <table width="100%" class="camposform">
                <tr>
                  ...
                  <td width="20">
							<% try {								
 		                        RadioHelper radioHelper1 = new RadioHelper();   
   								radioHelper1.setNombre(AWLiquidacionConsulta.IMPRIMIR_RECIBO);
                  			    radioHelper1.setId(AWLiquidacionConsulta.IMPRIMIR_RECIBO);  
                  			    radioHelper1.setValordefecto("SI"); 
							    radioHelper1.render(request,out);							    
						     }
						 		catch(HelperException re){
							 	out.println("ERROR " + re.getMessage());
						 	}
                  ...
                  <td width="16">
							<% try {
 		                        String defecto = (String)request.getSession().getAttribute(AWLiquidacionConsulta.IMPRIMIR_RECIBO);
 		                        if(defecto==null){
 		                        	request.getSession().setAttribute(AWLiquidacionConsulta.IMPRIMIR_RECIBO,"NO");
 		                        }
 		                        
 		                        RadioHelper radioHelper1 = new RadioHelper();   
   								radioHelper1.setNombre(AWLiquidacionConsulta.IMPRIMIR_RECIBO);
                  			    radioHelper1.setId(AWLiquidacionConsulta.IMPRIMIR_RECIBO);
                  			    radioHelper1.setValordefecto("NO");                  			    
							    radioHelper1.render(request,out);
						     }
						 		catch(HelperException re){
							 	out.println("ERROR " + re.getMessage());
...
%>
        ...
        <table width="100%" class="camposform">
            ...
            <tr>
              <td>&nbsp;</td>
              <td>Primer Apellido / Razón Social</td>
              <td>
              <% try {
          ...
          <table width="100%" class="camposform">
            <tr>
              <td width="20"><img src="<%=request.getContextPath()%>/jsp/images/ind_vinculo.gif" width="20" height="15"></td>
              <td width="140"><input name="imageField" type="image" src="<%=request.getContextPath()%>/jsp/images/btn_liquidar.gif" width="139" height="21" border="0"></td>
              <td>&nbsp;</td>
            </tr>
          </table>
