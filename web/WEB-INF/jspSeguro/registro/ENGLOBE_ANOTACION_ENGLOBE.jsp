...
<%
	...
	variosCiudadanosHelper.setPropertiesHandly(numFilas,tiposIDNatural,tiposPersona,tiposSexo,tiposIDJuridica,AWEnglobe.AGREGAR_1_REGISTRO_TABLA_CIUDADANOS_ENGLOBE,
																AWEnglobe.REMOVER_1_REGISTRO_TABLA_CIUDADANOS_ENGLOBE,
																AWEnglobe.AGREGAR_VARIOS_CIUDADANOS_ENGLOBE,
																AWEnglobe.ELIMINAR_CIUDADANO_ANOTACION_ENGLOBE,
																lciudadanos, AWEnglobe.VALIDAR_CIUDADANO_ENGLOBE, "ENGLOBE");
	...
	request.getSession().setAttribute("paramVistaAnterior","ENGLOBE_ANOTACION_ENGLOBE");
...
function Secuencia(text){
	document.ENGLOBE.SECUENCIA.value = text;
	cambiarAccion('REFRESCAR_ANOTACION_ENGLOBE');
}

function cambiarAccionAndSendTipoTarifa(text) {
	if(document.ENGLOBE.<%=CFolio.ANOTACION_NAT_JURIDICA_ESPECIFICACION_ID %>.value!=""){
		document.ENGLOBE.ACCION.value = text;
		document.ENGLOBE.action = 'calificacion.do';
		document.ENGLOBE.submit();
	}
}
...
<script type="text/javascript" >
	...
	function validarFecha(){
		if (document.ENGLOBE.<%=CSolicitudRegistro.CALENDAR%>.value.length>0){
			var index=document.ENGLOBE.<%=CSolicitudRegistro.CALENDAR%>.value.lastIndexOf('/')+1;
			if (index!=null){
				var fin=document.ENGLOBE.<%=CSolicitudRegistro.CALENDAR%>.value.length;
				var texto=document.ENGLOBE.<%=CSolicitudRegistro.CALENDAR%>.value.substring(index,fin);
				if (texto.length!=4){
					alert('Fecha incorrecta');
					document.ENGLOBE.<%=CSolicitudRegistro.CALENDAR%>.value='';
					document.ENGLOBE.<%=CSolicitudRegistro.CALENDAR%>.focus();
				}
			}
		}
		if (document.ENGLOBE.<%=CSolicitudRegistro.CALENDAR2%>.value.length>0){
			var index=document.ENGLOBE.<%=CSolicitudRegistro.CALENDAR2%>.value.lastIndexOf('/')+1;
			if (index!=null){
				var fin=document.ENGLOBE.<%=CSolicitudRegistro.CALENDAR2%>.value.length;
				var texto=document.ENGLOBE.<%=CSolicitudRegistro.CALENDAR2%>.value.substring(index,fin);
				if (texto.length!=4){
					alert('Fecha de la anotación incorrecta');
					document.ENGLOBE.<%=CSolicitudRegistro.CALENDAR2%>.value='';
					document.ENGLOBE.<%=CSolicitudRegistro.CALENDAR2%>.focus();
				}
			}
		}
	}
 ...
 <input type="hidden" name="POSSCROLL" id="POSSCROLL" value="<%=(request.getParameter("POSSCROLL")!=null?request.getParameter("POSSCROLL"):"")%>">
  ...
  <tr>
    ...
    <td background="<%=request.getContextPath()%>/jsp/images/tabla_gral_bgn002.gif"> <table border="0" cellpadding="0" cellspacing="0">
        <tr>
          <td background="<%=request.getContextPath()%>/jsp/images/tabla_gral_bgn001.gif" class="titulotbgral">ASISTENTE ENGLOBE - PASO 2 - CREAR ANOTACIÓN DE ENGLOBE</td>
...
if(webAnotacion!=null){
	...
	String sFechaAnotacion = DateFormatUtil.format(webAnotacion.getFechaRadicacion());
					...
					<td>Fecha de la anotaci&oacute;n&nbsp;
						<% try { 
								...
								textHelper.setCssClase("camposformtext"); 
	               			    ...
	               			    textHelper.setFuncion(" onkeypress=\"return valideDate(event,'"+CSolicitudRegistro.CALENDAR2+"');\" "
                                                    + " onChange=\"fixDate('"+CSolicitudRegistro.CALENDAR2+"')\" "
                                                    + "onBlur=\"javascript:validarFecha()\"" ); 
								textHelper.render(request,out); 
							} 
								catch(HelperException  re){ 
              ...
              <table width="100%" class="camposform">
				  ...
				  <tr>
					...
					<td width="40"><%try {
							...
							textHelper.setFuncion("onBlur=\"cambiarAccionAndSendTipoTarifa('"+AWCalificacion.CARGAR_DESCRIPCION_NATURALEZA_ENGLOBE+"')\"");
...
<%--/**
              ...
              <table width="100%" class="camposform">
                <tr>
                  ...
                  <td colspan='5'>Copiar comentario en el folio englobado ?</td>
                </tr>
                <tr>
                  <td width="20">&nbsp;</td>
                  <td width="20">
	...
	variosCiudadanosHelper.setPropertiesHandly(numFilas,tiposIDNatural,tiposPersona,tiposSexo,tiposIDJuridica,AWEnglobe.AGREGAR_1_REGISTRO_TABLA_CIUDADANOS_ENGLOBE,
																...
																AWEnglobe.AGREGAR_VARIOS_CIUDADANOS_ENGLOBE,
																AWEnglobe.ELIMINAR_CIUDADANO_ANOTACION_ENGLOBE,
																ciudadanos, AWEnglobe.VALIDAR_CIUDADANO_ENGLOBE, "ENGLOBE");				
		...
		<table width="100%" class="camposform">
          <tr>
            ...
            <td width="150"><a href="javascript:cambiarAccion('<%=AWEnglobe.ENGLOBE_ANOTACION%>')"><img src="<%=request.getContextPath()%>/jsp/images/btn_seguir.gif" width="139" height="21" border="0"></a></td>
			<!--SI SE QUIERE LA OPCIÓN DE SELECCIONAR LA ANOTACIÓN A PARTIR DE UNA ANOTACIÓN TEMPORAL SE DEBE DESELECCIONAR LA SIGUIENTE LINEA.
			 ...
			 -->
			<!--<td width="150"><a href="javascript:cambiarAccion('<%=AWEnglobe.ENGLOBE_SIN_ANOTACION%>')"><img src="<%=request.getContextPath()%>/jsp/images/btn_saltar_pagina.gif" name="Folio" width="150" height="21" border="0" id="Folio"></a></td>-->


					<!--BOTON REGRESAR-->                 
                    <TD width="150">
	                <a href="registro.englobar.escoger.folios.view?POSSCROLL=<%=(request.getParameter("POSSCROLL")!=null?request.getParameter("POSSCROLL"):"")%>">
