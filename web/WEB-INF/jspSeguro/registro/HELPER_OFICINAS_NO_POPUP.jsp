...
var cargaDesdeTexto = false;
   ...
   * @mantis 0013414: Acta - Requerimiento No 069_453_Código_Notaria_NC
   * */
   function cargarOficina(campo){
   		if (document.getElementById("<%=CFolio.ANOTACION_OFICINA_DOCUMENTO_ID_OFICINA%>").value==""){
   			alert("Debe seleccionar el tipo de oficina válido");
   		}else{
   			AWOficinas.cargarOficina(document.getElementById("<%=CFolio.ANOTACION_OFICINA_DOCUMENTO_ID_OFICINA%>").value,cargarCampoOficinas);
   		}
   }
   function cargarCampoOficinas(data){
   		if(data!=null){
   			document.getElementById("<%=CFolio.ANOTACION_OFICINA_DOCUMENTO_NUM%>").value = data.nombre;
                        document.getElementById("<%=CFolio.ANOTACION_OFICINA_DOCUMENTO_OFICINA_VERSION%>").value = data.version;
   		}else{
   ...
   function llenarCamposOficinas(idNumeroOficina,nombreOficina){
   		...
   		document.getElementById("<%=CFolio.ANOTACION_OFICINA_DOCUMENTO_NUM%>").value = nombreOficina;
...
<tr>
	...
	<table width="100%" id="Nacional2" class="camposform"
		...
		<tr>
            ...
            * @mantis 0013414: Acta - Requerimiento No 069_453_Código_Notaria_NC
            */
            textHelper.setFuncion("onblur=\"cargarOficina(this)\"");
            if(read){
	            textHelper.setReadonly(read);
            }else{
	            textHelper.setReadonly(false);            
