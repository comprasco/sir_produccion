...
<%
	...
	List anotacionesSeleccionadas = new java.util.ArrayList();
	...
	while(itFolios.hasNext()){
		...
		if(folio.getIdMatricula().equals(folioHeredado.getIdMatricula())){
			List anotacionesHeredadas = folioHeredado.getAnotacionesHeredadas();
			Iterator itAnotaciones = anotacionesHeredadas.iterator();
			while(itAnotaciones.hasNext()){
				WebAnotaHereda anotaHereda = (WebAnotaHereda)itAnotaciones.next();
				anotacionesSeleccionadas.add(anotaHereda.getIdAnotacion());
			}
			break;
		}
	}
...
<% 
	...
	if(llaves==null){
		...
		lla.setNombrePaginador("NOMBRE_PAGINADOR_ENGLOBE");
		lla.setNombreResultado("NOMBRE_RESULTADO_ENGLOBE");
		lla.setNombreNumPagina("NUM_PAGINA_ACTUAL_ENGLOBE");
		llaves.addLLave(lla);
		request.getSession().setAttribute(WebKeys.NOMBRE_LLAVES_MOSTRAR_FOLIO, llaves);
	}
	if(lla==null){
		lla=llaves.getLLave("NOMBRE_RESULTADO_ENGLOBE", "NOMBRE_PAGINADOR_ENGLOBE");
	}
...
function recargarAnotaciones(text) {
	if(text!='<%=WebKeys.SIN_SELECCIONAR%>'){
       document.ENGLOBE_ESCOGER_FOLIO.ACCION.value = '<%=AWEnglobe.RECARGAR_ANOTACIONES_DEFINITIVAS%>';
       document.ENGLOBE_ESCOGER_FOLIO.TARGET_SELECTIONCOLLECTOR.value = document.<%= "FORMA_CANCELACION" %>.TARGET_SELECTIONCOLLECTOR.value;
       document.ENGLOBE_ESCOGER_FOLIO.submit();
    }
}
</script>
    ...
    <td background="<%=request.getContextPath()%>/jsp/images/tabla_gral_bgn002.gif"> <table border="0" cellpadding="0" cellspacing="0">
        <tr>
          <td background="<%=request.getContextPath()%>/jsp/images/tabla_gral_bgn001.gif" class="titulotbgral">
            ASISTENTE ENGLOBE - PASO 3 - SELECCIONAR ANOTACIONES A HEREDAR
            </td>
    ...
    <td class="tdtablaanexa02"><table border="0" cellpadding="0" cellspacing="0" width="100%">
        ...
        <tr>
          ...
          <td valign="top" bgcolor="#79849B" class="tdtablacentral">
            <form action="englobe.do" method="post" name="ENGLOBE_ESCOGER_FOLIO" id="ENGLOBE_ESCOGER_FOLIO">
    		...
    		<table width="100%" border="0" cellpadding="0" cellspacing="2" class="camposform">
                  ...
                  <%
						...
						helperFolios.setShowInstruccion(false);
...
local_CheckBoxController = new CheckBoxMultiselectController();
			   ...
			   <%try{
						...
						List anotacionesInvalidas = (List) request.getSession().getAttribute(WebKeys.LISTA_ANOTACIONES_INVALIDAS);	
			   			...
			   			mFolio.setnombreNumAnotacionTemporal("NUM_A_TEMPORAL_ENGOBLE");
...
{
    document.ENGLOBE.TARGET_SELECTIONCOLLECTOR.value = document.<%= "FORMA_CANCELACION" %>.TARGET_SELECTIONCOLLECTOR.value;
    document.ENGLOBE.<%= CFolio.ID_MATRICULA %>.value = document.ENGLOBE_ESCOGER_FOLIO.<%= CFolio.ID_MATRICULA %>.value;
            ...
            <input type="hidden" name="POSSCROLL" id="POSSCROLL" value="<%=(request.getParameter("POSSCROLL")!=null?request.getParameter("POSSCROLL"):"")%>">
                ...
                <tr>
                  ...
                  <td width="20">
					<%
							...
							if(webEnglobe!=null && webEnglobe.getFoliosHeredados()!=null && webEnglobe.getFoliosHeredados().size()>0){
								WebFolioHeredado webFolioHeredado = (WebFolioHeredado)webEnglobe.getFoliosHeredados().get(0);
              ...
              <table width="100%" class="camposform">
                ...
                <tr>
                  ...
                  <td width="150"><a href="javascript:enviarDatos('<%=gov.sir.core.web.acciones.registro.AWEnglobe.HEREDAR_ANOTACION%>')"><img src="<%=request.getContextPath()%>/jsp/images/btn_seguir.gif" name="Folio" width="139" height="21" border="0" id="Folio"></td>
                  <td width="150"><a href="javascript:enviarDatos('<%=gov.sir.core.web.acciones.registro.AWEnglobe.GUARDAR_ANOTACIONES_HEREDADAS%>')"><img src="<%=request.getContextPath()%>/jsp/images/btn_guardar.gif" name="Folio" width="139" height="21" border="0" id="Folio"></td>


					<!--BOTON REGRESAR-->                 
                    <TD width="150">
	                <a href="registro.englobar.anotacion.englobe.view?POSSCROLL=<%=(request.getParameter("POSSCROLL")!=null?request.getParameter("POSSCROLL"):"")%>">
