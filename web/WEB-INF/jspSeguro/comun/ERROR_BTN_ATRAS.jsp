...
<% Exception exception = (Exception)session.getAttribute(SMARTKeys.EXCEPCION);
	...
	String vistaAnterior = (String)session.getAttribute(org.auriga.smart.SMARTKeys.VISTA_ANTERIOR);
        if("wait.view".equals(vistaAnterior)){
            vistaAnterior = "javascript:history.go(-2);";
        }else{
  ...
  <tr> 
    ...
    <td class="tdtablaanexa02"> 
			...
			<input name="Submit" type="submit" class="botontextual" onClick="MM_openBrWindow('excepcion.view','EXCEPCION','resizable=yes,scrollbars=yes,location=no,status=yes,menubar=no,copyhistory=no, width=640,height=400')" value="Ver la Excepci&oacute;n">
                         ...
                         * @Mantis: 003_589
                         -->
                        <% if(exception.getMessage()==null && session.getAttribute(WebKeys.IDENTIFICADOR_DE_SESSION) != null){ %>
                            <input name="Submit2" type="submit" class="botontextual" onClick="javascript:window.location.href='roles.view';" value="&laquo; Volver">
                        <%} else{ %>
                            <input name="Submit2" type="submit" class="botontextual" onClick="<%=vistaAnterior%>" value="&laquo; Volver">
                        <%} %>
			</td>
			</tr>
			</table></td>
