...
<%          
            ...
            dirHelper.setNombreFormaEdicionDireccion("EDITAR");
...
function cambiarAccion(text) {
	document.EDITAR.ACCION.value = text;
}
...
function Secuencia(text){
	document.EDITAR.<%=CAnotacionCiudadano.SECUENCIA%>.value = text;
...
function quitarDireccion(pos,accion){
	if(confirm("Esta seguro que desea eliminar la dirección ?")){
		document.EDITAR.POSICION.value = pos;
...
function locacionTipoPredio(nombre,valor,dimensiones){
	...
	if(document.EDITAR.<%=CFolio.FOLIO_TIPO_PREDIO%>.value == '<%=gov.sir.core.negocio.modelo.constantes.CTipoPredio.TIPO_URBANO%>'){	
...
function consultarFolio(text) {
	document.EDITAR.action = 'certificado.do';
	document.EDITAR.ACCION.value = text;
	document.EDITAR.submit();
}
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	...
	<tr>
		...
		<table border="0" cellpadding="0" cellspacing="0" width="100%">
			<form action="antiguosistema.do" name="EDITAR" id="EDITAR" method="post"><input
            ...
            if (idMatricula.length() != 0 && session.getAttribute(CFolio.FOLIO_CREADO_HOJA_RUTA) == null) {
			...
			<tr>
                ...
                %>
						...
						<%}
							...
							try {
				                textHelper.setNombre(CFolio.SELECCIONAR_FOLIO + AWModificarFolio.FOLIO_COMPLEMENTACION);
				                textHelper.setCssClase("campositem");
				                textHelper.setId(CFolio.SELECCIONAR_FOLIO + AWModificarFolio.FOLIO_COMPLEMENTACION);
				                textHelper.setReadonly(true);
				                textHelper.render(request, out);
				            } catch (HelperException re) {
				                out.println("ERROR " + re.getMessage());
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	...
	<tr>
		...
		<%
			try {
				if(esCorreccion){
					dirHelper.setMostrarAgregarNueva(false);
				}
		...
		<table width="100%" class="camposform">
			<tr>
				...
				<td width="140"><input name="imageField" type="image"
					...
					onClick="cambiarAccionSubmit( '<%=gov.sir.core.web.acciones.antiguosistema.AWAntiguoSistema.TERMINAR_EDITAR_FOLIO%>')" width="139"
					height="21" border="0"></td>
				<td width="140"><input name="imageField" type="image"
					src="<%=request.getContextPath()%>/jsp/images/btn_editar_anotaciones.gif"
					onClick="cambiarAccionSubmit( '<%=gov.sir.core.web.acciones.antiguosistema.AWAntiguoSistema.EDITAR_ANOTACIONES%>')" width="139"
					height="21" border="0"></td>
...
<%try {
                ...
                helper.setNombreFormulario("EDITAR");
