...
<% 
    ...
    //ver is la oficina es internacional o naciol
        ...
        String tipoChecked = (String) session.getAttribute(AWDevolucion.CHECK_APODERADO);
        
        String tipoCheckedEdition = (String) session.getAttribute(AWDevolucion.CHECK_APODERADO + "E");
        
        String isNotificarUsuario = (String) session.getAttribute(AWDevolucion.IS_NOTIFICAR_USUARIO);
        ...
        boolean isUsuarioEdition = false;
        boolean isOficinaEdition = false;
        boolean isCorreoEdition = false;
        String idNot = (String) request.getSession().getAttribute(AWDevolucion.MODIFY_NOTIFICATION);
        
        if(idNot != null && !idNot.isEmpty()){
            
        String isNotificarUsuarioE = (String) session.getAttribute(AWDevolucion.IS_NOTIFICAR_USUARIO + "E");
        if(isNotificarUsuarioE != null && !isNotificarUsuarioE.isEmpty()){
            if(isNotificarUsuarioE.equals(CDevoluciones.NOT_USUARIO)){
                isUsuarioEdition = true;
                isOficinaEdition = false; 
            } else if(isNotificarUsuarioE.equals(CDevoluciones.NOT_OFICINA)){
                isUsuarioEdition = false;
                isOficinaEdition = true; 
            } else{
                isUsuarioEdition = false;
                isOficinaEdition = false;
            }
        ...
        String isNotificarCorreoE = (String) session.getAttribute(AWDevolucion.IS_NOTIFICAR_CORREO + "E");
        
        if(isNotificarCorreoE != null && !isNotificarCorreoE.isEmpty()){
            if(isNotificarCorreoE.equals(CDevoluciones.CORREO_ELECTRONICO)){ 
                isCorreoEdition = true;
            } else{
                isCorreoEdition = false;
            }
        }
        }
        ...
        String alert = null;
        try{
            alert = (String) session.getAttribute(CDevoluciones.ALERTA_VENCIMIENTO); 
        } catch(Exception e){
        ...
        String isNotificarCorreo = (String) session.getAttribute(AWDevolucion.IS_NOTIFICAR_CORREO);
        boolean isCorreo = false;
        if(isNotificarCorreo != null && !isNotificarCorreo.isEmpty()){
            if(isNotificarCorreo.equals(CDevoluciones.CORREO_ELECTRONICO)){
                isCorreo = true;
            } else{
                isCorreo = false;
            }
        ...
        ListaElementoHelper destinoHelper = new ListaElementoHelper();
        List destino = (List)session.getAttribute(AWDevolucion.DESTINATARIOS); 
	if(destino == null){
		destino = new ArrayList();
	}
	destinoHelper.setOrdenar(false);
	destinoHelper.setTipos(destino);
	destinoHelper.setCssClase("camposformtext");
        
        ListaElementoHelper tipoHelper = new ListaElementoHelper();
        List tipos = (List)session.getAttribute(AWDevolucion.TIPOS); 
	if(tipos == null){
		tipos = new ArrayList(); 
	}
	tipoHelper.setOrdenar(false);
	tipoHelper.setTipos(tipos);
	tipoHelper.setCssClase("camposformtext");
        
        TextHelper txtHelper = new TextHelper(); 
        
        ListaElementoHelper documentoHelper = new ListaElementoHelper();   
        List tiposDoc = (List) session.getAttribute(AWDevolucion.TIPOS_DOC);  
	if(tiposDoc == null){
		tiposDoc = new ArrayList(); 
	}
	documentoHelper.setOrdenar(false);
	documentoHelper.setTipos(tiposDoc);
	documentoHelper.setCssClase("camposformtext");
...
function cambiarAccion(text) {
	document.NOTIFICAR_NOTA_DEVOLUTIVA.ACCION.value = text;
    	document.NOTIFICAR_NOTA_DEVOLUTIVA.submit();
}
...
function crudNotification(text,idNotification){
    document.NOTIFICACION.ACCION.value = text;
    document.getElementById('<%=AWDevolucion.ID_TO_MODIFY%>').value = idNotification;
    document.NOTIFICACION.submit();
}
function validarFecha(){
		if (document.NOTIFICAR_NOTA_DEVOLUTIVA.<%=AWDevolucion.FECHA_NOTIFICACION%>.value.length>0){
			var index=document.NOTIFICAR_NOTA_DEVOLUTIVA.<%=AWDevolucion.FECHA_NOTIFICACION%>.value.lastIndexOf('/')+1;
			if (index!=null){
				var fin=document.NOTIFICAR_NOTA_DEVOLUTIVA.<%=AWDevolucion.FECHA_NOTIFICACION%>.value.length;
				var texto=document.NOTIFICAR_NOTA_DEVOLUTIVA.<%=AWDevolucion.FECHA_NOTIFICACION%>.value.substring(index,fin);
				if (texto.length!=4){
					alert('Fecha incorrecta');
					document.NOTIFICAR_NOTA_DEVOLUTIVA.<%=AWDevolucion.FECHA_NOTIFICACION%>.value='';
					document.NOTIFICAR_NOTA_DEVOLUTIVA.<%=AWDevolucion.FECHA_NOTIFICACION%>.focus();
				}
			}
		}
	}
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
   ...
   <table border="0" cellpadding="0" cellspacing="0" width="100%">
                    ...
                    <tr>
                        ...
                        <td class="tdtablaanexa02">
                            <br>
                            <p style="color:red;"><strong><%=alert%></strong></p>
                            </td>
   ...
   %>  
    ...
    <input type="hidden" id="<%=AWDevolucion.ID_TO_MODIFY%>" name="<%=AWDevolucion.ID_TO_MODIFY%>">     
    <table width="100%" class="camposform">
        <tr>
            <td>
            ...
            <tr>
                ...
                <td>
                    ...
                    <%
                        ...
                        request.getSession().setAttribute(AWDevolucion.FECHA_NOTIFICACION, formattedDate);
                        /**
                        try {

                            TextHelper notifyDate = new TextHelper();
                            notifyDate.setId(AWDevolucion.FECHA_NOTIFICACION);
                            notifyDate.setCssClase("campositem");
                            notifyDate.setEditable(false);
                            notifyDate.setNombre(AWDevolucion.FECHA_NOTIFICACION);
                            notifyDate.setFuncion("  onkeypress=\"return valideDate(event,'"+AWDevolucion.FECHA_NOTIFICACION+"');\" "
                                                        + " onChange=\"fixDate('"+AWDevolucion.FECHA_NOTIFICACION+"')\"   onBlur=\"javascript:validarFecha()\"" ); 
                            notifyDate.render(request,out);  
                        } catch (HelperException re) { 
                            out.println("ERROR " + re.getMessage());
                        }
                        */
                        %>
                        <td class="campositem" width="120">
                           &nbsp; <%=formattedDate%>&nbsp;
                        </td>
                    <%-- 
                    <td>
                    <a
                      href="javascript:NewCal('<%=AWDevolucion.FECHA_NOTIFICACION%>','ddmmmyyyy',true,24)"
                    >
                      <img
            ...
            <tr>
                <td>Destinatario: </td>
                                    <td><%
                        try{
                    destinoHelper.setNombre(AWDevolucion.DESTINO_NOTIFICACION);
                    destinoHelper.setId(AWDevolucion.DESTINO_NOTIFICACION);
                    destinoHelper.setFuncion("onChange=\"cambiarAccionNotificacion('CARGAR_CONTEXTO')\"");
                    destinoHelper.render(request,out);  
            ...
            <tr>
                <td>Tipo de Notificaci&oacute;n: </td>
                                    <td><%
                       try{
                    tipoHelper.setNombre(AWDevolucion.TIPO_NOTIFICACION);
                    tipoHelper.setId(AWDevolucion.TIPO_NOTIFICACION);
                    tipoHelper.setFuncion("onChange=\"cambiarAccionNotificacion('CARGAR_CONTEXTO')\"");
                    tipoHelper.render(request,out);  
                 } catch(HelperException he){
                     out.println("ERROR " + he.getMessage());
                 }   
            %></td>
              <%if(isCorreo){%>
                                    ...
                                    <%
                         try {
			                textHelper.setNombre(AWDevolucion.CORREO); 
			                textHelper.setSize("50");
			                textHelper.setCssClase("camposformtext");
			                textHelper.setId(AWDevolucion.CORREO);
			                textHelper.render(request, out);
			            } catch (HelperException re) {
			                out.println("ERROR " + re.getMessage());
			            }
              ...
              <%}%>
                 ...
                 <tr>
                     ...
                     if(tipoChecked != null){
                    if (tipoChecked.equals(AWDevolucion.APODERADO_SI)) {
                          checkedSi = "CHECKED";
                    } else{
                          checkedNo = "CHECKED";
                    }}
                    %>
                <input type="radio" name="<%= AWDevolucion.CHECK_APODERADO%>" value="<%= AWDevolucion.APODERADO_SI%>" <%= checkedSi%> onClick="cambiarAccionNotificacion('<%=AWDevolucion.APODERADO_SI%>');"/> Si
                &nbsp;
                <input type="radio" name="<%= AWDevolucion.CHECK_APODERADO%>" value="<%= AWDevolucion.APODERADO_NO%>" <%= checkedNo%> onClick="cambiarAccionNotificacion('<%=AWDevolucion.APODERADO_NO%>');"/> No </td> 
                 </tr>
                 ...
                 <tr>
                     ...
                     <td><%
                         try {
			                textHelper.setNombre(AWDevolucion.NOMBRES_NOTIFICACION);
			                textHelper.setSize("30");
			                textHelper.setCssClase("camposformtext");
			                textHelper.setId(AWDevolucion.NOMBRES_NOTIFICACION);
			                textHelper.render(request, out);
			            } catch (HelperException re) {
			                out.println("ERROR " + re.getMessage());
			            }
                 ...
                 <tr>
                     ...
                     <td><%
                         try {
			                textHelper.setNombre(AWDevolucion.APELLIDOS_NOTIFICACION); 
			                textHelper.setSize("30");
			                textHelper.setCssClase("camposformtext");
			                textHelper.setId(AWDevolucion.APELLIDOS_NOTIFICACION);
			                textHelper.render(request, out);
			            } catch (HelperException re) {
			                out.println("ERROR " + re.getMessage());
			            }
                 ...
                 <tr>
                     ...
                     <td><%
                            try{
                        documentoHelper.setNombre(AWDevolucion.TIPO_DOCUMENTO);
                        documentoHelper.setId(AWDevolucion.TIPO_DOCUMENTO);
                        documentoHelper.render(request,out);  
                     } catch(HelperException he){
                         out.println("ERROR " + he.getMessage());
                     }                   
                     %></td>    

                     <td width="200">Numero de Documento: </td>
                     <td><%
                            try{
                        TextHelper numDocHelper = new TextHelper();
                        numDocHelper.setNombre(AWDevolucion.NUMERO_DOCUMENTO); 
                        numDocHelper.setSize("30");
                        numDocHelper.setCssClase("camposformtext");
                        numDocHelper.setId(AWDevolucion.NUMERO_DOCUMENTO);
                        numDocHelper.setFuncion("onkeypress=\"return valideKey(event,'"+ AWDevolucion.NUMERO_DOCUMENTO +"');\"");
                        numDocHelper.render(request, out);
                     } catch(HelperException he){
                         out.println("ERROR " + he.getMessage());
                     }                   
                     %></td>    
                ...
                <tr>
                 ...
                 <tr>
                     ...
                     <td><%
                            try{
                        textHelper.setNombre(AWDevolucion.DIRECCION); 
                        textHelper.setSize("30");
                        textHelper.setCssClase("camposformtext");
                        textHelper.setId(AWDevolucion.DIRECCION);
                        textHelper.render(request, out);
                     } catch(HelperException he){
                         out.println("ERROR " + he.getMessage());
                     }                   
                 ...
                 <tr>
                     ...
                     <td><%
                            try{
                        textHelper.setNombre(AWDevolucion.TELEFONO); 
                        textHelper.setSize("30");
                        textHelper.setCssClase("camposformtext");
                        textHelper.setId(AWDevolucion.TELEFONO);
                        textHelper.render(request, out);
                     } catch(HelperException he){
                         out.println("ERROR " + he.getMessage());
                     }                   
                 ...
                 <tr>
                     <td><a href="javascript:cambiarAccionNotificacion('<%=AWDevolucion.AGREGAR_NOTIFICACION%>')"><img src="<%=request.getContextPath()%>/jsp/images/btn_agregar.gif" border="0"></a> </td>
                 </tr>
            </table>
            </td>
        </tr>
    ...
    <%
            ...
            while(itNot.hasNext()){
                ...
                boolean isOffice = false;
                    ...
                    String modifyNot = (String) request.getSession().getAttribute(AWDevolucion.MODIFY_NOTIFICATION);
                    boolean modify = false;
                    if(modifyNot != null && modifyNot.equals(idNotification)){ 
                        modify = true;
                    } else{
                        modify = false;
                    }
    ...
    <table width="100%" class="camposform"> 
            ...
            <td colspan="4">&nbsp;</td>
            <td align="right">
                    <a href="javascript:crudNotification('<%=AWDevolucion.GUARDAR_NOTIFICACION%>','<%=idNotification%>')"><img src="<%=request.getContextPath()%>/jsp/images/btn_mini_guardar.gif"  border="0"></a>
            </td>
        </tr>
        <tr>
            <td>
              ...
              <%}%>
                ...
                <tr>
                 ...
                 <tr>
                    ...
                    <tr>
                        <td width="40%">
                        <a href="javascript:crudNotification('<%=AWDevolucion.EDITAR_NOTIFICACION%>','<%=idNotification%>')"><img src="<%=request.getContextPath()%>/jsp/images/btn_short_editar_on.gif" width="35" height="25" border="0"></a>
                        </td>
                         <td width="40%">
                        <a href="javascript:crudNotification('<%=AWDevolucion.ELIMINAR_NOTIFICACION%>','<%=idNotification%>')"><img src="<%=request.getContextPath()%>/jsp/images/btn_short_eliminar.gif" width="35" height="25" border="0"></a>
                        </td>
                    </tr>
                </table>
                     </td>
            ...
            <tr>
                ...
                <td>
                    ...
                    <%
                        ...
                        Date dateObjM = calendar.getTime();
                        String formattedDateM = DateFormatUtil.format(dateObjM);
                        request.getSession().setAttribute(AWDevolucion.FECHA_NOTIFICACION + "E", formattedDateM);
                        /**
                        try {
            ...
            <tr>
                <td>Destinatario: </td>
                                    <td><%
                         request.getSession().setAttribute(AWDevolucion.DESTINO_NOTIFICACION + "E", notify.getDestino());
                        try{
                    destinoHelper.setNombre(AWDevolucion.DESTINO_NOTIFICACION + "E");
                    destinoHelper.setId(AWDevolucion.DESTINO_NOTIFICACION + "E");
                    destinoHelper.setFuncion("onChange=\"cambiarAccionNotificacion('CARGAR_CONTEXTO')\"");
                    destinoHelper.render(request,out);  
                 } catch(HelperException he){
                     out.println("ERROR " + he.getMessage());
            ...
            <tr>
                <td>Tipo de Notificaci&oacute;n: </td>
                                    <td><%
                        request.getSession().setAttribute(AWDevolucion.TIPO_NOTIFICACION + "E", notify.getTipo());
                       try{
                    tipoHelper.setNombre(AWDevolucion.TIPO_NOTIFICACION + "E");
                    tipoHelper.setId(AWDevolucion.TIPO_NOTIFICACION + "E");
                    tipoHelper.setFuncion("onChange=\"cambiarAccionNotificacion('CARGAR_CONTEXTO')\"");
                    tipoHelper.render(request,out);  
                 } catch(HelperException he){
                     out.println("ERROR " + he.getMessage());
            ...
            %></td>
              <%if(isCorreoEdition || notify.getCorreo() != null){%>
                                    <td>Correo Electr&oacute;nico: </td>
                                    <td>
                                    <%
                                        request.getSession().setAttribute(AWDevolucion.CORREO + "E", notify.getCorreo());
                         try {
			                textHelper.setNombre(AWDevolucion.CORREO + "E"); 
			                textHelper.setSize("50");
			                textHelper.setCssClase("camposformtext");
			                textHelper.setId(AWDevolucion.CORREO + "E");
			                textHelper.render(request, out);
			            } catch (HelperException re) {
			                out.println("ERROR " + re.getMessage());
			            }
              ...
              <%}%>
                 ...
                 <% if(isUsuarioEdition || notify.getDestino().equals(CDevoluciones.USUARIO)){%>
                 <tr>
                     <td>Apoderado: </td>
                     <td>
                     <%String checkedSi = "", checkedNo = "";
                    if (tipoCheckedEdition == null || tipoCheckedEdition.equals(AWDevolucion.APODERADO_SI)) {
                          checkedSi = "CHECKED";
                    } else{
                          checkedNo = "CHECKED";
                    }
                    %>
                <input type="radio" name="<%= AWDevolucion.CHECK_APODERADO + "E"%>" value="<%= AWDevolucion.APODERADO_SI%>" <%= checkedSi%> onClick="cambiarAccionNotificacion('<%=AWDevolucion.APODERADO_SI%>');"/> Si
                &nbsp;
                <input type="radio" name="<%= AWDevolucion.CHECK_APODERADO + "E"%>" value="<%= AWDevolucion.APODERADO_NO%>" <%= checkedNo%> onClick="cambiarAccionNotificacion('<%=AWDevolucion.APODERADO_NO%>');"/> No </td> 
                 </tr>
                 
                 <tr>
                     <td>Nombre: </td>
                     <td><%
                         request.getSession().setAttribute(AWDevolucion.NOMBRES_NOTIFICACION + "E", notify.getNombres()); 
                         try {
			                textHelper.setNombre(AWDevolucion.NOMBRES_NOTIFICACION + "E");
			                textHelper.setSize("30");
			                textHelper.setCssClase("camposformtext");
			                textHelper.setId(AWDevolucion.NOMBRES_NOTIFICACION + "E");
			                textHelper.render(request, out);
			            } catch (HelperException re) {
			                out.println("ERROR " + re.getMessage());
			            }
                 ...
                 <tr>
                     <td>Apellidos: </td>
                     <td><%
                         request.getSession().setAttribute(AWDevolucion.APELLIDOS_NOTIFICACION + "E", notify.getApellidos()); 
                         try {
			                textHelper.setNombre(AWDevolucion.APELLIDOS_NOTIFICACION + "E"); 
			                textHelper.setSize("30");
			                textHelper.setCssClase("camposformtext");
			                textHelper.setId(AWDevolucion.APELLIDOS_NOTIFICACION + "E");
			                textHelper.render(request, out);
			            } catch (HelperException re) {
			                out.println("ERROR " + re.getMessage());
			            }
                 ...
                 <tr>
                     <td>Tipo de Documento: </td>
                     <td><%
                         request.getSession().setAttribute(AWDevolucion.TIPO_DOCUMENTO + "E", notify.getTipoDocumento()); 
                            try{
                        documentoHelper.setNombre(AWDevolucion.TIPO_DOCUMENTO + "E");
                        documentoHelper.setId(AWDevolucion.TIPO_DOCUMENTO + "E");
                        documentoHelper.render(request,out);  
                     } catch(HelperException he){
                         out.println("ERROR " + he.getMessage());
                     }                   
                     ...
                     <td width="200">Numero de Documento: </td>
                     <td><%
                         request.getSession().setAttribute(AWDevolucion.NUMERO_DOCUMENTO + "E", notify.getDocumento()); 
                            try{
                        TextHelper numDocHelper = new TextHelper();
                        numDocHelper.setNombre(AWDevolucion.NUMERO_DOCUMENTO + "E"); 
                        numDocHelper.setSize("30");
                        numDocHelper.setCssClase("camposformtext");
                        numDocHelper.setId(AWDevolucion.NUMERO_DOCUMENTO + "E");
                        numDocHelper.setFuncion("onkeypress=\"return valideKey(event,'"+ AWDevolucion.NUMERO_DOCUMENTO + "E" +"');\"");
                        numDocHelper.render(request, out);
                     } catch(HelperException he){
                         out.println("ERROR " + he.getMessage());
                     }                   
                 ...
                 <%} else if(isOficinaEdition || notify.getDestino().equals(CDevoluciones.OFICINA_ORIGEN)){%>
                ...
                <tr>
                 ...
                 <tr>
                     <td>Direcci&oacute;n: </td>
                     <td><%
                         request.getSession().setAttribute(AWDevolucion.DIRECCION + "E", notify.getDireccion()); 
                            try{
                        textHelper.setNombre(AWDevolucion.DIRECCION + "E"); 
                        textHelper.setSize("30");
                        textHelper.setCssClase("camposformtext");
                        textHelper.setId(AWDevolucion.DIRECCION + "E");
                        textHelper.render(request, out);
                     } catch(HelperException he){
                         out.println("ERROR " + he.getMessage());
                     }                   
                 ...
                 <tr>
                     <td>Telefono: </td>
                     <td><%
                         request.getSession().setAttribute(AWDevolucion.TELEFONO + "E", notify.getTelefono()); 
                            try{
                        textHelper.setNombre(AWDevolucion.TELEFONO + "E"); 
                        textHelper.setSize("30");
                        textHelper.setCssClase("camposformtext");
                        textHelper.setId(AWDevolucion.TELEFONO + "E");
                        textHelper.render(request, out);
                     } catch(HelperException he){
                         out.println("ERROR " + he.getMessage());
                     }                   
    ...
    %>
        ...
        <tr>
          ...
          <table width="100%" class="camposform">
            <tr>
                ...
                <td><a href="javascript:cambiarAccionNotificacion('NOTIFICAR_NOTA_CONFIRMAR')"><img src="<%=request.getContextPath()%>/jsp/images/btn_aceptar.gif" border="0"></a> </td>

              <form name="NOTIFICAR_NOTA_DEVOLUTIVA" method="post" action="devolucion.do">
        ...
        <tr>
          ...
          <td><img name="tabla_central_pint_r3_c7" src="<%=request.getContextPath()%>/jsp/images/tabla_central_pint_r3_c7.gif" width="11" height="6" border="0" alt=""></td>
             ...
             <%
		try{
                    helper.setNombreFormulario("NOTIFICAR_NOTA_DEVOLUTIVA");
...
<%if(carga){%>
<script>cambiarAccion('<%=AWDevolucion.NOTIFICAR_NOTA_DEVOLUTIVA%>')</script>
<%}%>