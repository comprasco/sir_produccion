...
<% ListaElementoHelper certHelper = new ListaElementoHelper();
   ...
   <SCRIPT>
	...
	function cambiarAccion(text) {
       document.CERTIFICADO_ESPECIAL.ACCION.value = text;
       document.CERTIFICADO_ESPECIAL.submit();
	}
		
	function quitar(text) {
       document.CERTIFICADO_ESPECIAL.ITEM.value = text;
       cambiarAccion('ELIMINAR');
	}
	
	function datosAntSistema(ocultar){
		document.CERTIFICADO_ESPECIAL.OCULTAR.value = ocultar;
		cambiarAccion('RECARGAR');
	}
	
	
	function incrementarSecuencial(text){
		document.CERTIFICADO_ESPECIAL.action = 'certificado.do'
		document.CERTIFICADO_ESPECIAL.ACCION.value = text;
		document.CERTIFICADO_ESPECIAL.submit();
	}
        
        function generarCertificadoEspecial(text){
                document.CERTIFICADO_ESPECIAL.action = 'certificadoEspecial.do'
                document.CERTIFICADO_ESPECIAL.ACCION.value = text;
                document.CERTIFICADO_ESPECIAL.submit();
        }
	...
	{
		document.CERTIFICADO_ESPECIAL.ACCION.value='<%=CSolicitudRegistro.PRESERVAR_INFO%>';
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
    ...
    <form action="certificadoEspecial.do" method="post" name="CERTIFICADO_ESPECIAL" id="CERTIFICADO_ESPECIAL">
...
%>
                     ...
                     try {
                            ...
                            checkedNal = tipoOficinaChecked != null && tipoOficinaChecked.equals(CCertificado.OFICINA_ORIGEN_NACIONAL);
	 		                   ...
	 		                   if(tipos != null){
	 		                    ...
	 		                    TextHelper inmediato = new TextHelper();
                                            inmediato.setEditable(false);
                                            inmediato.setNombre("TIPO_CERTIFICADO");
                                            inmediato.setId("TIPO_CERTIFICADO");
                                            inmediato.setCssClase("camposformtext");
                                            inmediato.setFuncion("value=\"INMEDIATO\"");
                                            inmediato.render(request,out);
							}
    ...
    <input name="Ver" type="hidden" id="nom_vereda" value="">              
		...
		{ 
		  ...
		  helper.setNombreFormulario("CERTIFICADO_ESPECIAL");
