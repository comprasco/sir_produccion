...
<%
	...
	String matricula = "";
	if(session.getAttribute(AWImpresionFolio.IMPRESION_CERTIFICADO_EJECUTADA) != null){ 
		impresionOK = true;
		matricula = (String)session.getAttribute(CTurno.ID_TURNO);
		session.removeAttribute(CTurno.ID_TURNO);
		session.removeAttribute(AWImpresionFolio.IMPRESION_CERTIFICADO_EJECUTADA);
	}
...
function cambiarAccionSubmit2(text1,text2) {
        document.BUSCAR.action = text2;
	document.BUSCAR.<%= WebKeys.ACCION %>.value = text1;
}
function generarCertificadoEspecial(text){
                document.BUSCAR.action = 'impresionFolioEspecial.do';
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr> 
    ...
    <td class="tdtablaanexa02">
        ...
        <input type="hidden" name="<%= WebKeys.ACCION %>" id="<%=  AWImpresionFolio.IMPRIMIR_CERTIFICADO%>" value="<%= AWImpresionFolio.IMPRIMIR_CERTIFICADO %>">
         ...
         <%
            ...
            %>  
              <tr>
               <td width="20"></td>
               <td >Se realizó satisfactoriamente la impresión del certificado <%= matricula %></td>
             </tr>
            <%
            }
            %>         
          ...
          <td allign="left" width="250">Causal de Reimpresión:</td>
            ...
            <% 
             ...
             { 
				...
				java.util.List tiposNotas = (java.util.List) session.getAttribute(AWImpresionFolio.LISTA_INFORMATIVAS_REIMPRESION);
			 ...
			 <td width="80">
                ...
                <table width="100%" class="camposform">
                  <tr>
                    ...
                    <td width="10%">
                    <input name="imageField" type="image" onClick="cambiarAccion('<%=AWImpresionFolio.IMPRIMIR_CERTIFICADO%>');"  src="<%=request.getContextPath()%>/jsp/images/btn_reimprimir.gif" width="150" height="21" border="0">
                    </td>
                    <!--
                      * @author      :   Julio Alcázar Rivas
                    ...
                    !-->
                    <td width="13%">
                        <input name="imageField1" type="image" onClick="cambiarAccion('<%=AWImpresionFolio.IMPRIMIR_CERTIFICADO_PERTENENCIA%>');" src="<%=request.getContextPath()%>/jsp/images/btn_oficiopertenencia.gif" width="150" height="21" border="0" >
                    </td>
                    <td width="10%">
                        <input name="imageField2" type="image" onClick="cambiarAccionSubmit2('<%=AWTrasladoTurno.CONSULTAR_TURNO_CERTIFICADO_CAMBIO_MATRICULA%>','trasladoTurno.do');" src="<%=request.getContextPath()%>/jsp/images/btn_cambiar_matricula.gif" width="180" height="21" border="0" >
                    </td>
...
<%if(recarga){%>
	<script>
		cambiarAccionSubmit('<%=AWImpresionFolio.OBTENER_IMPRESORAS_CIRCULO_CERTIFICADO%>');
	</script>
<%}%>
</body>
