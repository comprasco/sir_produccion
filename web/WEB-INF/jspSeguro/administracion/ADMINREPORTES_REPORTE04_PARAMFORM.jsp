...
<script type="text/javascript">
    ...
    var REPORTE_E04__SEND_ACTION       = "<%= AWReportes.REPORTE_E04__SEND_ACTION %>";
    var REPORTE_E04__BACK_ACTION       = "<%= AWReportes.REPORTE_E04__BACK_ACTION %>"; // the same
    var REPORTE_E04_LOADCIRCULOSBYUSUARIO = "<%= AWReportes.REPORTE_E04_LOADCIRCULOSBYUSUARIO %>"; // the same
...
<script type="text/javascript">
    ...
    function getUsuariosSeleccionados(){
        ...
        var listaUsuarios = document.getElementById('<%=AWReportes.REPORTE_E04__PARAM_USUARIONOMBRE %>').options;
        for(var i = 0; i < listaUsuarios.length; i = i + 1){
            var usuario = listaUsuarios[i];
            if(usuario.selected == true){
                if(usuariosSeleccionados.length > 0){
                    usuariosSeleccionados = usuariosSeleccionados + ',';
                }
                usuariosSeleccionados = usuariosSeleccionados + usuario.value;
            }
        }
        document.getElementById('<%=AWReportes.REPORTE_E04__PARAM_USUARIOSNOMBRE %>').value = usuariosSeleccionados;
    }
</script>
<script type="text/javascript">
   ...
   function js_OnEvent( formName, actionValue ) {
     ...
     htmlForm = new LocalForm( formName );
     htmlForm.setValue( actionField, actionValue );
     if(actionValue == REPORTE_E04__SEND_ACTION){
...
%>
<script type="text/javascript">
  js_OnEvent( "INITIAL_TIME_LOAD", REPORTE_E04_LOADCIRCULOSBYUSUARIO );
</script>
<%
  }
%>
        ...
        <table border="0" cellpadding="0" cellspacing="0" width="101%">
          ...
          <tr>
            ...
            <td background="<%=request.getContextPath()%>/jsp/images/tabla_central_bgn003.gif">
                <table border="0" cellpadding="0" cellspacing="0">
                    <tr>
                      <td background="<%=request.getContextPath()%>/jsp/images/tabla_central_bgn001.gif" class="titulotbcentral">Reporte E04</td>
            ...
            <td valign="top" bgcolor="#79849B" class="tdtablacentral">
                ...
                <table width="100%" border="0" cellpadding="0" cellspacing="0">
                  ...
                  <form action="reportes.do" method="POST" name="BUSCAR" id="BUSCAR">
                      <input  type="hidden" name="<%=WebKeys.ACCION%>" id="<%=WebKeys.ACCION %>" value="">
                      <input  type="hidden" name="<%=AWReportes.REPORTE_E04__PARAM_USUARIOSNOMBRE %>" id="<%=AWReportes.REPORTE_E04__PARAM_USUARIOSNOMBRE %>" value="">
                      <input  type="hidden" name="<%=AWReportes.REPORTE_E04__PARAM_USUARIOLOG %>" id="<%=AWReportes.REPORTE_E04__PARAM_USUARIOLOG %>" value="<%=String.valueOf(param_UsuarioSir.getUsername()) %>">
                      ...
                      <table width="100%" class="camposform">
                            <tr>
                                ...
                                <td width="200">
                                    <%
                                        try {
                                            if( null == session.getAttribute( AWReportes.REPORTE_E04__PARAM_PFECHAINI ) ) {
                                                session.setAttribute( AWReportes.REPORTE_E04__PARAM_PFECHAINI, fechaAct );
                                            }
                                            TextHelper textHelper = new TextHelper();
                                            textHelper.setId( AWReportes.REPORTE_E04__PARAM_PFECHAINI );
                                            textHelper.setNombre( AWReportes.REPORTE_E04__PARAM_PFECHAINI );
                                    ...
                                    %>
                                    <a href="javascript:NewCal('<%=AWReportes.REPORTE_E04__PARAM_PFECHAINI%>','ddmmmyyyy',true,24)"><img src="<%=request.getContextPath()%>/jsp/images/ico_calendario.gif" alt="Fecha" width="15" height="21" border="0" onClick="javascript:Valores('<%=request.getContextPath()%>/')"></a>
                                </td>
                                <td>&nbsp;</td>
                            </tr>
                            <tr>
                                ...
                                <td width="200">
                                    <%
                                        try {
                                            if( null == session.getAttribute( AWReportes.REPORTE_E04__PARAM_PFECHAFIN ) ) {
                                                session.setAttribute( AWReportes.REPORTE_E04__PARAM_PFECHAFIN, fechaAct );
                                            }
                                            TextHelper textHelper = new TextHelper();
                                            textHelper.setId( AWReportes.REPORTE_E04__PARAM_PFECHAFIN );
                                            textHelper.setNombre( AWReportes.REPORTE_E04__PARAM_PFECHAFIN );
                                            textHelper.setCssClase("camposformtext");
                                            textHelper.render(request,out);
                                        }catch(HelperException re){
                                            out.println("ERROR " + re.getMessage());
                                    ...
                                    %>
                                    <a href="javascript:NewCal('<%=AWReportes.REPORTE_E04__PARAM_PFECHAFIN%>','ddmmmyyyy',true,24)"><img src="<%=request.getContextPath()%>/jsp/images/ico_calendario.gif" alt="Fecha" width="15" height="21" border="0" onClick="javascript:Valores('<%=request.getContextPath()%>/')"></a>
                                </td>
                                <td>&nbsp;</td>
                            </tr>
                            <tr>
                                ...
                                <td width="200">
                                    <%
                                        try {
                                            ...
                                            circuloHelper.setId( AWReportes.REPORTE_E04__PARAM_IDCIRCULO );
                                            circuloHelper.setNombre( AWReportes.REPORTE_E04__PARAM_IDCIRCULO );
                                            circuloHelper.setTipos((List) session.getAttribute(AWReportes.REPORTE_XX__ITEM_CIRCULOSBYUSUARIO));
                                            circuloHelper.setCssClase("camposformtext");
                                            circuloHelper.setFuncion("onChange=\"cambiarAccionAndSend('"+AWReportes.REPORTE_E04_LOADUSUARIOSBYCIRCULO+"')\"");
                            ...
                            <tr>
                                <td width="20"><img src="<%=request.getContextPath()%>/jsp/images/ind_campotxt.gif" width="20" height="15"></td>
                                <td align="left" width="150"><div align="right" >Usuario:</div></td>
                                <td width="200">
                                    <%
                                        try{
                                            List usuarios = null;
                                            if(session.getAttribute(AWReportes.USUARIOS_CONSULTAR_POR_CIRCULO) != null){
                                                usuarios = (List) session.getAttribute(AWReportes.USUARIOS_CONSULTAR_POR_CIRCULO);
                                            }else{
                                                usuarios = new ArrayList();
                                            }
                                            
                                            ListaElementoHelper usuariosHelper = new ListaElementoHelper();
                                            usuariosHelper.setId(AWReportes.REPORTE_E04__PARAM_USUARIONOMBRE);
                                            usuariosHelper.setNombre(AWReportes.REPORTE_E04__PARAM_USUARIONOMBRE);
                                            usuariosHelper.setTipos(usuarios);
                                            usuariosHelper.setTipoMultiple(true);
                ...
                <table width="100%" class="camposform">
                    <tr>
                        <td width="155">
                            <a href="javascript:js_OnEvent( 'BUSCAR', REPORTE_E04__BACK_ACTION );">
                                <img alt="regresar"   src="<%=request.getContextPath()%>/jsp/images/btn_regresar.gif" width="139" height="21" border="0" />
                            </a>
                        </td>
                        <td>
                            <a href="javascript:js_OnEvent( 'BUSCAR', REPORTE_E04__SEND_ACTION );">
