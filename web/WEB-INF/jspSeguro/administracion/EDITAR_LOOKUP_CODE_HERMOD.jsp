...
<%
	...
	String nombre= request.getParameter(COPLookupCodes.NOMBRE_OPLOOKUP_CODE);
	

	if(nombre!=null){
		//obtener el lookuptype
		OPLookupCodes dato=null;
		...
		for(Iterator iter = tipos.iterator(); iter.hasNext() && !termino;){
    		OPLookupCodes tmp = (OPLookupCodes)iter.next();
			if(tmp.getCodigo().equals(nombre)){
				dato=tmp;
				termino=true;
			}
		}
		//colocar las variables en session para que salga como los valores default
		session.setAttribute(COPLookupCodes.NOMBRE_OPLOOKUP_CODE, dato.getCodigo());
        session.setAttribute(COPLookupCodes.VALOR_OPLOOKUP_CODE, dato.getValor());
		session.setAttribute(COPLookupCodes.DESCRIPCION_OPLOOKUP_CODE, dato.getDescripcion());

		session.setAttribute(COPLookupCodes.NOMBRE_OPLOOKUP_CODE_EDITADO, dato.getCodigo());
	}
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
    ...
    <table width="100%" class="camposform">
        <tr>
            ...
            <td width="150"><a href="javascript:cambiarAccion('<%=AWAdministracionHermod.UPDATE_OPLOOKUP_CODE%>')"><img src="<%=request.getContextPath()%>/jsp/images/btn_editar.gif" width="150" height="20" border="0"></a></td>
