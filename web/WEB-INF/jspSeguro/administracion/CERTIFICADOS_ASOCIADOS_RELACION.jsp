...
<%	 
	...
	Hashtable turnosRegistro = (Hashtable) session.getAttribute(WebKeys.LISTA_TURNOS_RELACION_DETALLE);
	boolean relacionConsultada = false;
	if(session.getAttribute(WebKeys.LISTA_TURNOS_RELACION)!=null){
		List turnosRelacion = (List)session.getAttribute(AWRelacion.LISTA_TURNOS_RELACION);
		if(turnosRelacion.size()>0){
			relacionConsultada = true;
		}
	}
...
//	session.removeAttribute(AWRelacion.AVANCE_TURNOS_AUTOMATICAMENTE);
...
function cambiarAccionAtras(text) {       
	document.BUSCAR.action = 'relacion.do';
    document.BUSCAR.<%= WebKeys.ACCION %>.value = text;
    document.BUSCAR.submit();
}
...
function cargaInicial() {
	document.BUSCAR.action = 'relacion.do';
	document.BUSCAR.<%= WebKeys.ACCION %>.value = '<%= AWRelacion.VER_DETALLE_RELACION %>';
	document.BUSCAR.submit();
}
</script>
...
<input  type="hidden" name="<%= AWRelacion.ID_RELACION %>" id="<%= AWRelacion.ID_RELACION %>" value="<%=(String)session.getAttribute(AWRelacion.ID_RELACION)%>">
 <input  type="hidden" name="<%= AWRelacion.RELACION_CONSULTADA %>" id="<%= AWRelacion.RELACION_CONSULTADA %>" value="<%=Boolean.toString(relacionConsultada)%>">
...
<!--                  <td width="18%" align="left" class="campositem">&nbsp;</td>
              ...
              <table width="100%" class="camposform">
                <tr>
                  <td width="15%"><a href="javascript:cambiarAccionAtras('<%= AWRelacion.INGRESAR_RELACION%>')"><img src="<%=request.getContextPath()%>/jsp/images/btn_volver.gif" width="150" height="21" border="0"></a></td>
                  <td width="55%">&nbsp;</td>
                </tr>
              </table>
            </form>
