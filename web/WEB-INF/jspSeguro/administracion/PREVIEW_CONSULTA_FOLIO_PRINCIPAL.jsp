...
<% 
	...
	Boolean temp=(Boolean)session.getAttribute(WebKeys.MAYOR_EXTENSION_FOLIO_PREVIEW);	
	
	List foliosDerivadoHijos = (List) request.getSession().getAttribute(WebKeys.LISTA_FOLIOS_DERIVADO_HIJO_PREVIEW);
    
	List foliosDerivadoPadre = (List) request.getSession().getAttribute(WebKeys.LISTA_FOLIOS_DERIVADO_PADRE_PREVIEW);

	List turnosFolioTramite = (List) request.getSession().getAttribute(WebKeys.LISTA_TURNOS_FOLIO_TRAMITE_PREVIEW);
	...
	if(request.getSession().getAttribute("folioPosibleTemporal")!=null){
		Folio folio = (Folio)request.getSession().getAttribute(WebKeys.FOLIO_AUXILIAR);
	...
	java.math.BigDecimal totalAnotaciones = (java.math.BigDecimal) request.getSession().getAttribute(WebKeys.TOTAL_ANOTACIONES_PREVIEW);          	 	
	List gravamenes = (List) request.getSession().getAttribute(WebKeys.LISTA_ANOTACIONES_GRAVAMEN_PREVIEW);          	 	
	List medCautelares = (List) request.getSession().getAttribute(WebKeys.LISTA_ANOTACIONES_MEDIDAS_CAUTELARES_PREVIEW); 
	List falsaTradicion = (List) request.getSession().getAttribute(WebKeys.LISTA_ANOTACIONES_FALSA_TRADICION_PREVIEW);
	List anotacionesInvalidas = (List) request.getSession().getAttribute(WebKeys.LISTA_ANOTACIONES_INVALIDAS_PREVIEW);	
	List anotacionesPatrimonioFamiliar = (List) request.getSession().getAttribute(WebKeys.LISTA_ANOTACIONES_PATRIMONIO_FAMILIAR_PREVIEW);	
	List anotacionesAfectacionVivienda = (List) request.getSession().getAttribute(WebKeys.LISTA_ANOTACIONES_AFECTACION_VIVIENDA_PREVIEW);	
	...
	turnoTramite= (Turno)session.getAttribute(WebKeys.TURNO_TRAMITE_PREVIEW);
	
	Usuario usuarioBloqueo= null;
	usuarioBloqueo= (Usuario) session.getAttribute(WebKeys.USUARIO_BLOQUEO_PREVIEW);
	
	Turno turnoDeuda=null;
	turnoDeuda= (Turno)session.getAttribute(WebKeys.TURNO_DEUDA_PREVIEW);    
        /**
