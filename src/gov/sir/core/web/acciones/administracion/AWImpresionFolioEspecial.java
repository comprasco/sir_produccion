 ...
 * @author jmendez
 */
public class AWImpresionFolioEspecial extends SoporteAccionWeb {
	...
	public static final String IMP_MASIVA_SIMPLE_FOLIO_FTP = "IMP_MASIVA_SIMPLE_FOLIO_FTP";
        ...
        /** Constante que identifica que se reimprime un certificado especial*/
       ...
       private StringBuffer listaFolios;
        ...
        public Evento perform(HttpServletRequest request) throws AccionWebException {
            ...
            if ((accion == null) || (accion.trim().length() == 0)) {
            ...
            } else if(accion.equals(AWImpresionFolioEspecial.CERTIFICADO_ESPECIAL)){
                return null;
            } else if (accion.equals(AWImpresionFolioEspecial.IMPRIMIR_FOLIO)) {
                return imprimirFolio(request);
            } else if (accion.equals(AWImpresionFolioEspecial.IMPRIMIR_CERTIFICADO)) {
                return imprimirCertificadoEspecial(request);
            } else if (accion.equals(AWImpresionFolioEspecial.IMPRIMIR_CERTIFICADO_PERTENENCIA)) {
                return imprimirCertificadoEspecial(request);
            } else if (accion.equals(AWImpresionFolioEspecial.REIMPRIMIR_RECIBO)) {
                ...
                return obtenerImpresoras(request);
            } else if (accion.equals(AWImpresionFolioEspecial.OBTENER_ULTIMO_TURNO_IMPRESO)) {
                return consultarUltimoReciboImpreso(request);
            } else if (accion.equals(AWImpresionFolioEspecial.ULTIMO_TURNO_IMPRESO_PROCESO)) {
                return consultarUltimoTurnoImpresoProceso(request);
            } else if (accion.equals(AWImpresionFolioEspecial.REIMPRIMIR_RECIBOS)) {
                return reimprimirRecibos(request);
            } else if (accion.equals(AWImpresionFolioEspecial.REGRESAR_REIMPRIMIR_RECIBO)) {
    ...
    private EvnImprimirFolio reimprimirRecibos(HttpServletRequest request) throws AccionWebException {
        ...
        String[] idTurnos = request.getParameterValues(AWImpresionFolioEspecial.LISTA_RECIBOS_IMPRIMIR);
    ...
    private Evento consultarUltimoTurnoImpresoProceso(HttpServletRequest request) throws AccionWebException {
        ...
        request.getSession().removeAttribute(AWImpresionFolioEspecial.LISTA_TURNOS_ASOCIADOS_REIMPRESION);
	 ...
	 * @throws AccionWebException
	 */
	private EvnImprimirFolio imprimirCertificadoEspecial(HttpServletRequest request) throws AccionWebException {
		HttpSession session = request.getSession();
		...
		evento.setTipoNota(tipoNota);
                evento.setCertificadoEspecial(true);
                cleaner(request);
		return evento;
	}
...
////////////////////////////////////////////////////////////////////////
	...
	private EvnImprimirFolio limpiarSesion(HttpServletRequest request) throws AccionWebException {
		HttpSession session = request.getSession();
		session.removeAttribute(AWImpresionFolioEspecial.IMPRESION_FOLIO_EJECUTADA);
		session.removeAttribute(CFolio.ID_MATRICULA);
		session.removeAttribute(CTurno.ID_TURNO);
		session.removeAttribute(AWImpresionFolioEspecial.LISTA_TURNOS_ASOCIADOS_REIMPRESION);
		...
		session.removeAttribute(AWImpresionFolioEspecial.PROGRAMAR_TAREA_EXITO);
                session.removeAttribute(AWImpresionFolioEspecial.IMP_MASIVA_SIMPLE_FOLIO_FTP);
		return null;
	}
...
//                        reader.close();
		...
		return new EvnImprimirFolio(usuarioAuriga, listaFolios, nombreArchivo, AWImpresionFolioEspecial.AGREGAR_DE_ARCHIVO);
    }
        
        private void cleaner(HttpServletRequest request){
            request.getSession().removeAttribute(CTurno.ID_TURNO);
            request.getSession().removeAttribute(WebKeys.IMPRESORA);
            request.getSession().removeAttribute(WebKeys.NUMERO_COPIAS_IMPRESION);
            request.getSession().removeAttribute(CTipoNota.ID_TIPO_NOTA);
            request.getSession().removeAttribute(CTipoNota.DESCRIPCION);
        }
        /**
         * @author     : Ellery David Robles Gómez.
         * @change     : Metodo que se encarga de leer el archivo csv con sus folios y cargarlos a un StringBuffer.
...
//            try {
                ...
                listaFolios2 = (StringBuffer) session.getAttribute(AWImpresionFolioEspecial.LISTA_MATRICULAS);
//            } catch (Exception ex) {
//                exception.addError("El archivo no existe.");
//                if (exception.getErrores().size() > 0) { throw exception; }
//            }
            ...
            return new EvnImprimirFolio(usuarioAuriga, nombreTarea, fechaTarea, listaFolios2, AWImpresionFolioEspecial.IMP_MASIVA_SIMPLE_FOLIO, EvnImprimirFolio.PROGRAMAR_TAREA);
        }
	//	//////////////////////////////////////////////////////////////////////
	////////////////////////////////////////////////////////////////////////
	////////////////////////////////////////////////////////////////////////
    ...
    public void doEnd(HttpServletRequest request, EventoRespuesta evento) {
        ...
        if (respuesta != null) {
            ...
            if (tipoEvento.equals(EvnRespImprimirFolio.IMPRESION_FOLIO_OK)) {
                session.setAttribute(AWImpresionFolioEspecial.IMPRESION_FOLIO_EJECUTADA, new Boolean(true));
                return;
                ...
                session.setAttribute(AWImpresionFolioEspecial.LISTA_ESTACIONES, respuesta.getPayload());
                return;
            } else if (tipoEvento.equals(EvnRespImprimirFolio.IMPRESION_CERTIFICADO_OK)) {
                session.setAttribute(AWImpresionFolioEspecial.IMPRESION_CERTIFICADO_EJECUTADA, new Boolean(true));
                return;
                ...
                session.setAttribute(AWImpresionFolioEspecial.LISTA_INFORMATIVAS_REIMPRESION, respuesta.getListaNotasReimpresion());
                return;
                ...
                session.setAttribute(AWImpresionFolioEspecial.LISTA_TURNOS_ASOCIADOS_REIMPRESION, respuesta.getTurnosAsociados());
            } else if (tipoEvento.equals(EvnRespImprimirFolio.AGREGAR_DE_ARCHIVO)) {
                session.setAttribute(AWImpresionFolioEspecial.NOMBRE_ARCHIVO_CSV, respuesta.getNombreArchivo());
                session.setAttribute(AWImpresionFolioEspecial.LISTA_MATRICULAS, respuesta.getListaFolios());
            } else if (tipoEvento.equals(EvnRespImprimirFolio.PROGRAMAR_TAREA_OK)) {
                session.setAttribute(AWImpresionFolioEspecial.PROGRAMAR_TAREA_EXITO, respuesta.getExito());
                session.setAttribute(AWImpresionFolioEspecial.IMP_MASIVA_SIMPLE_FOLIO_FTP, respuesta.getImp_masiva_simple_folio_ftp());
                return;
            }
        }
    }
