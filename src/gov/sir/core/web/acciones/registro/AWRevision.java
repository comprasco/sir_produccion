...
public class AWRevision extends SoporteAccionWeb {
	...
	public Evento perform(HttpServletRequest request)
		throws AccionWebException {
			...
			/*}else if (accion.equals(TOMAR_TURNO)){
				...
				return aprobarRevision(request);			
	...
	private Evento tomarTurno(HttpServletRequest request) {
		...
		return new EvnRevision (usuario,turno,usuarioNeg);
	}
	...
	private Evento revisarFolio(HttpServletRequest request) {
		...
		return new EvnRevision(usuarioAuriga, EvnFolio.CONSULTA, folio, u);
	}

	private void eliminarInfoBasicaFolio(HttpServletRequest request) {
		request.getSession().removeAttribute(CFolio.ID_MATRICULA);
		request.getSession().removeAttribute(AWRevision.FOLIO_COD_CATASTRAL);
		request.getSession().removeAttribute(AWRevision.FOLIO_LOCACION_ID_DEPTO);
		request.getSession().removeAttribute(AWRevision.FOLIO_LOCACION_NOM_DEPTO);
		request.getSession().removeAttribute(AWRevision.FOLIO_LOCACION_ID_MUNIC);
		request.getSession().removeAttribute(AWRevision.FOLIO_LOCACION_NOM_MUNIC);
		request.getSession().removeAttribute(AWRevision.FOLIO_LOCACION_ID_VEREDA);
		request.getSession().removeAttribute(AWRevision.FOLIO_LOCACION_NOM_VEREDA);
		request.getSession().removeAttribute(AWRevision.FOLIO_TIPO_PREDIO);
		request.getSession().removeAttribute(AWRevision.FOLIO_COMPLEMENTACION);
		request.getSession().removeAttribute(AWRevision.FOLIO_LINDERO);
		request.getSession().removeAttribute(AWRevision.FOLIO_COD_CATASTRAL);
		request.getSession().removeAttribute(AWRevision.FOLIO_EJE1);
		request.getSession().removeAttribute(AWRevision.FOLIO_VALOR1);
		request.getSession().removeAttribute(AWRevision.FOLIO_EJE2);
		request.getSession().removeAttribute(AWRevision.FOLIO_VALOR2);
		request.getSession().removeAttribute(AWRevision.FOLIO_COMPLEMENTO_DIR);
		request.getSession().removeAttribute(AWRevision.FOLIO_COD_DOCUMENTO);
		request.getSession().removeAttribute(AWRevision.FOLIO_FECHA_APERTURA);
		request.getSession().removeAttribute(AWRevision.FOLIO_TIPO_PREDIO);
		request.getSession().removeAttribute(AWRevision.FOLIO_ESTADO_FOLIO);
	...
	private void eliminarInfoBasicaAnotacion(HttpServletRequest request) {
		request.getSession().removeAttribute(AWRevision.ANOTACION_OFICINA_DOCUMENTO_NUM);
		request.getSession().removeAttribute(AWRevision.ANOTACION_OFICINA_DOCUMENTO_TIPO);
		request.getSession().removeAttribute(AWRevision.ANOTACION_OFICINA_DOCUMENTO_ID_TIPO);
		request.getSession().removeAttribute(AWRevision.ANOTACION_OFICINA_DOCUMENTO_ID_OFICINA);
		request.getSession().removeAttribute(AWRevision.ANOTACION_ID_ANOTACION);
		request.getSession().removeAttribute(AWRevision.ANOTACION_COMENTARIO_ESPECIFICACION);
		request.getSession().removeAttribute(AWRevision.ANOTACION_FECHA_DOCUMENTO);
		request.getSession().removeAttribute(AWRevision.ANOTACION_FECHA_RADICACION);
		request.getSession().removeAttribute(AWRevision.ANOTACION_NAT_JURIDICA_ESPECIFICACION);
		request.getSession().removeAttribute(AWRevision.ANOTACION_TIPO_DOCUMENTO);
		request.getSession().removeAttribute(AWRevision.ANOTACION_VALOR_ESPECIFICACION);
		request.getSession().removeAttribute(AWRevision.ANOTACION_NUM_DOCUMENTO);
		request.getSession().removeAttribute(AWRevision.ANOTACION_OFICINA_DOCUMENTO_ID_DEPTO);
		request.getSession().removeAttribute(AWRevision.ANOTACION_OFICINA_DOCUMENTO_NOM_DEPTO);
		request.getSession().removeAttribute(AWRevision.ANOTACION_OFICINA_DOCUMENTO_ID_MUNIC);
		request.getSession().removeAttribute(AWRevision.ANOTACION_OFICINA_DOCUMENTO_NOM_MUNIC);
		request.getSession().removeAttribute(AWRevision.ANOTACION_OFICINA_DOCUMENTO_ID_VEREDA);
		request.getSession().removeAttribute(AWRevision.ANOTACION_OFICINA_DOCUMENTO_NOM_VEREDA);
		request.getSession().removeAttribute(AWRevision.ANOTACION_OFICINA_DOCUMENTO_ID_VEREDA);
		request.getSession().removeAttribute(AWRevision.ANOTACION_NAT_JURIDICA_ESPECIFICACION_ID);
		request.getSession().removeAttribute(AWRevision.ANOTACION_NAT_JURIDICA_ESPECIFICACION_NOM);
	...
	private void eliminarInfoBasicaCiudadano(HttpServletRequest request) {
		request.getSession().removeAttribute(AWRevision.ANOTACION_TIPO_ID_PERSONA);
		request.getSession().removeAttribute(AWRevision.ANOTACION_NUM_ID_PERSONA);
		request.getSession().removeAttribute(AWRevision.ANOTACION_APELLIDO_1_PERSONA);
		request.getSession().removeAttribute(AWRevision.ANOTACION_APELLIDO_2_PERSONA);
		request.getSession().removeAttribute(AWRevision.ANOTACION_NOMBRES_PERSONA);
		request.getSession().removeAttribute(AWRevision.ANOTACION_TIPO_INTER_ASOCIACION);
		request.getSession().removeAttribute(AWRevision.ANOTACION_PORCENTAJE_ASOCIACION);
		request.getSession().removeAttribute(AWRevision.ANOTACION_ES_PROPIETARIO_ASOCIACION);
	}
	 ...
	 * @return
	 */
	private Evento aprobarRevision(HttpServletRequest request) {
		org.auriga.core.modelo.transferObjects.Usuario usuario = (org.auriga.core.modelo.transferObjects.Usuario) request.getSession().getAttribute(SMARTKeys.USUARIO_EN_SESION);
		Turno turno = (Turno)request.getSession().getAttribute(WebKeys.TURNO);
		...
		return new EvnRevision(usuario,userNeg, turno,fase,EvnRevision.WF_CONFIRMAR, CAvanzarTurno.AVANZAR_CUALQUIERA);
	 ...
	 * @return
	 */
	private Evento negarRevision(HttpServletRequest request) {
		org.auriga.core.modelo.transferObjects.Usuario usuario = (org.auriga.core.modelo.transferObjects.Usuario) request.getSession().getAttribute(SMARTKeys.USUARIO_EN_SESION);
		Turno turno = (Turno)request.getSession().getAttribute(WebKeys.TURNO);
		Fase fase = (Fase)request.getSession().getAttribute(WebKeys.FASE);
		Usuario userNeg = (gov.sir.core.negocio.modelo.Usuario)request.getSession().getAttribute(WebKeys.USUARIO);
		return new EvnRevision(usuario,userNeg, turno,fase,EvnRevision.WF_NEGAR, CAvanzarTurno.AVANZAR_POP);
	...
	private Evento confirmarDevolucion(HttpServletRequest request) {
		...
		Fase fase = (Fase)request.getSession().getAttribute(WebKeys.FASE);
		Usuario userNeg = (gov.sir.core.negocio.modelo.Usuario)request.getSession().getAttribute(WebKeys.USUARIO);
		return new EvnRevision(usuario,userNeg, turno,fase,EvnRevision.WF_CONFIRMAR, CAvanzarTurno.AVANZAR_ELIMINANDO_PUSH);
	...
	public void doEnd(HttpServletRequest request,EventoRespuesta evento) {
		...
		else if(evento instanceof EvnRespRevision){
			EvnRespRevision respuesta = (EvnRespRevision) evento;
			if(respuesta != null){
				if(respuesta.getTipoEvento().equals(EvnRespRevision.SEGREGACION_MASIVA)){
					request.getSession().setAttribute(WebKeys.LISTA_FOLIOS_DERIVADOS,respuesta.getFoliosDerivados());
				} 
				else if (respuesta.getTipoEvento().equals(EvnRespRevision.FOLIO_TEMPORAL)){
					request.getSession().setAttribute(WebKeys.FOLIO,respuesta.getFolio());
