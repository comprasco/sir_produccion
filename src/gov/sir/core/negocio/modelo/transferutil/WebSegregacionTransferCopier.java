 ...
 *
 */
public class WebSegregacionTransferCopier {

	public static org.auriga.core.modelo.TransferObject deepCopy( gov.sir.core.negocio.modelo.jdogenie.WebSegregacionEnhanced enhancedObject, java.util.HashMap cache)
	{
		gov.sir.core.negocio.modelo.WebSegregacion transferObject = new gov.sir.core.negocio.modelo.WebSegregacion();
		cache.put(enhancedObject, transferObject );
		
		  // FoliosDerivado
    	List foliosDerivado = null;
  	try
  	{
  	foliosDerivado = enhancedObject.getFoliosDerivados();
  	} catch (Throwable th) {}
	  	if (foliosDerivado != null)
	  	{
		  	for(Iterator itera = foliosDerivado.iterator();
		  			itera.hasNext();)
		  	{
		  		boolean assigned = false;
		  		gov.sir.core.negocio.modelo.jdogenie.WebFolioDerivadoEnhanced foliosDerivadoEnh = (gov.sir.core.negocio.modelo.jdogenie.WebFolioDerivadoEnhanced)itera.next();
		  				  				  		if (!assigned)
		  		{
		  		gov.sir.core.negocio.modelo.WebFolioDerivado objTo = (gov.sir.core.negocio.modelo.WebFolioDerivado)cache.get(foliosDerivadoEnh);
		  		if (objTo == null)
		  		{
		  		objTo = (gov.sir.core.negocio.modelo.WebFolioDerivado)WebFolioDerivadoTransferCopier.deepCopy(foliosDerivadoEnh, cache);
		  		}
		  		transferObject.addFoliosDerivado(objTo);
		  		}
		  				  	}
		}
	...
	public static gov.sir.core.negocio.modelo.jdogenie.Enhanced deepCopy( gov.sir.core.negocio.modelo.WebSegregacion transferObject, java.util.HashMap cache)
	{
		gov.sir.core.negocio.modelo.jdogenie.WebSegregacionEnhanced enhancedObject = new gov.sir.core.negocio.modelo.jdogenie.WebSegregacionEnhanced();
		cache.put(transferObject, enhancedObject);
		  // FoliosDerivado
    	List foliosDerivado = null;
  	try
  	{
  	foliosDerivado = transferObject.getFoliosDerivados();
  	} catch (Throwable th) { }
	  	if (foliosDerivado != null)
	  	{
		  	for(Iterator itera = foliosDerivado.iterator();
		  	...
		  	{
		  		boolean assigned = false;
		  		gov.sir.core.negocio.modelo.WebFolioDerivado foliosDerivadoto = (gov.sir.core.negocio.modelo.WebFolioDerivado)itera.next();
		  				  				  		if (!assigned)
		  		{
		  				  		gov.sir.core.negocio.modelo.jdogenie.WebFolioDerivadoEnhanced objEn = (gov.sir.core.negocio.modelo.jdogenie.WebFolioDerivadoEnhanced)cache.get(foliosDerivadoto);
		  		if (objEn == null)
		  		{
		  			objEn = (gov.sir.core.negocio.modelo.jdogenie.WebFolioDerivadoEnhanced)WebFolioDerivadoTransferCopier.deepCopy(foliosDerivadoto, cache);
		  		}
		  		enhancedObject.addFoliosDerivado(objEn);
		  				  		}
		  				  	}
		}
