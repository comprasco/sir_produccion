...
public class IDidTurnoComparatorRelacion implements Comparator 
{
	public int compare(Object turno, Object anotherTurno)
	{
		try
		{
			...
			Turno turno2 = (Turno) anotherTurno;
                        int comp = 0;
                        if(turno1.getIdFase().equals("REG_ENTREGA") || turno1.getIdFase().equals("REG_ENTREGA_EXTERNO") || turno1.getIdFase().equals("COS_ENTREGAR_ASOCIADOS") ){
                            String relacion1 = turno1.getIdRelacionActual();
                            String relacion2 = turno2.getIdRelacionActual();
                            if(relacion1!=null && relacion2!=null){
                                Integer anioR1 = new Integer(relacion1.split("-")[0]);
                                Integer anioR2 = new Integer(relacion2.split("-")[0]);
                                if(anioR1.compareTo(anioR2)==0){
                                    if(relacion1.split("-").length > 1 && relacion2.split("-").length > 1){
                                        Integer idR1 = new Integer(relacion1.split("-")[2]);
                                        Integer idR2 = new Integer(relacion2.split("-")[2]);
                                        comp = idR1.compareTo(idR2);
                                        if(comp!=0){
                                            return comp;
                                        }
                                    }
                                }else{
                                    return anioR1.compareTo(anioR2);
                                }
                            }
                        }
